...
const networkFeeUsd = () => (config.PAPER_BASE_FEE_LAMPORTS + config.PAPER_PRIORITY_FEE_LAMPORTS + config.PAPER_JITO_TIP_LAMPORTS) / 1_000_000_000 * config.PAPER_SOL_USD;
...
export class PaperBroker {
  ...
  buy(token: string, symbol: string, observedPrice: number, requestedUsd: number, pairUrl: string): Position {
    ...
    const dexFeeUsd = bps(requestedUsd, config.PAPER_DEX_FEE_BPS), chainFeeUsd = networkFeeUsd(), cost = requestedUsd + dexFeeUsd + chainFeeUsd;
  ...
  sell(position: Position, observedPrice: number, reason = 'manual sell'): PaperFill {
    ...
    const dexFeeUsd = bps(grossUsd, config.PAPER_DEX_FEE_BPS), chainFeeUsd = networkFeeUsd(), proceeds = grossUsd - dexFeeUsd - chainFeeUsd;

...
const env = z.object({
  ...
  MAX_FDV_LIQUIDITY_RATIO: z.coerce.number().min(1).default(25), MAX_POSITION_USD: z.coerce.number().positive().default(50),
  ...
  MAX_OPEN_POSITIONS: z.coerce.number().int().min(1).max(20).default(5), MAX_TOP_HOLDER_PERCENT: z.coerce.number().min(1).max(100).default(25),

...
const env = z.object({
  ...
  MIN_SCORE: z.coerce.number().min(1).max(100).default(60), MIN_LIQUIDITY_USD: z.coerce.number().min(1000).default(15000),
  MAX_FDV_LIQUIDITY_RATIO: z.coerce.number().min(1).default(50), MAX_POSITION_USD: z.coerce.number().positive().default(50),
  ...
  MAX_OPEN_POSITIONS: z.coerce.number().int().min(1).max(20).default(5), MAX_TOP_HOLDER_PERCENT: z.coerce.number().min(1).max(100).default(35),
  MAX_QUOTE_PRICE_IMPACT_PERCENT: z.coerce.number().min(.1).max(100).default(25), CONTROL_API_KEY: z.string().min(16).optional(),

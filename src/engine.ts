...
export class ScannerEngine {
  ...
  openPaperPosition(candidate: Candidate): Position {
    if (this.positions.some(position => position.token === candidate.baseToken.address)) throw new Error('Position already exists');
    const position = { token: candidate.baseToken.address, symbol: candidate.baseToken.symbol, entryPrice: Number(candidate.priceUsd), highPrice: Number(candidate.priceUsd), amountUsd: config.MAX_POSITION_USD, openedAt: Date.now(), pairUrl: candidate.url };
    if (!Number.isFinite(position.entryPrice) || position.entryPrice <= 0) throw new Error('Missing usable price');
